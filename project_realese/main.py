...
def ssh_value():
    ...
    file.write("passwd=" + ui.SSHpass.text() + "\n")
...
def checking_ip_ddns():
    bol = is_valid_ip(ui.DDNSipnet.text()) & is_valid_ip(ui.DDNSmask.text()) & is_valid_ip(ui.DDNSserip.text()) \
          & is_valid_ip(ui.DDNSranstart.text()) & is_valid_ip(ui.DDNSranend.text())
    return bol


# Проверка ip адреса для VPN
def checking_ip_vpn():
    bolv = is_valid_ip(ui.VPNuip.text()) & is_valid_ip(ui.VPNip_mask.text())
    return bolv


# Проверка количества выбраных настроек
def count_settings():
    i = 0
...
def progress():
    if count_settings() > 0:
        ui.progressBar.setStyleSheet("color: rgba(0,0,0,255); \n"
...
def click():
    progress()
    ui.progressBar.setProperty("value", 1)
    counter = 100 / count_settings()
    counter1 = 0
    ...
    if ui.DDNS.isChecked():
        ...
        if ui.DDNSUmol.isChecked() == False:
            if checking_ip_ddns():
                ...
                sleep(2)
                counter1 += counter
                ui.progressBar.setProperty("value", counter1)
            else:
                ip_not_corecct()
        # если по умолчанию
        else:
